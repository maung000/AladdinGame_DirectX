...
bool Entity::isInCamera(Camera * camera)
{
	if ((this->posX >= camera->GetBound().left) && (this->posX <= camera->GetBound().right)
		&& (this->posY >= camera->GetBound().top) && (this->posY <= camera->GetBound().bottom))
		return true;
	return false;
}